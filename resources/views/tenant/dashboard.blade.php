...
<body class="bg-slate-50 text-slate-900 min-h-screen">
    ...
    <div class="flex">
        ...
        <aside class="w-64 bg-white border-r border-slate-200 min-h-[calc(100vh-64px)] p-6 hidden lg:block">
            <nav class="space-y-2">
                <a href="#" class="flex items-center gap-3 px-4 py-3 bg-indigo-50 text-indigo-700 rounded-xl font-bold transition">
        ...
        <main class="flex-1 p-8 lg:p-12">
            <header class="mb-12 flex justify-between items-end">
                ...
                <button class="bg-indigo-600 text-white px-6 py-3 rounded-xl font-bold shadow-lg shadow-indigo-200 hover:scale-105 transition">Add New Dish</button>
            </header>

            <!-- Menu CMS List -->
            <div class="bg-white rounded-3xl shadow-xl shadow-slate-200/50 border border-slate-100 overflow-hidden mb-12">
                        ...
                        <tbody class="divide-y divide-slate-100">
                            @foreach($menus as $menu)
                                <tr class="hover:bg-slate-50/50 transition">
                                    <td class="px-8 py-6">
                                        <div class="flex items-center gap-4">
                                            <img src="{{ $menu->image_url ?? 'https://images.unsplash.com/photo-1546069901-ba9599a7e63c?auto=format&fit=crop&q=80&w=800' }}" class="w-14 h-14 rounded-xl object-cover" alt="">
                                            <div>
                                                <p class="font-bold text-slate-900">{{ $menu->name }}</p>
                                                <p class="text-xs text-slate-500 line-clamp-1">{{ $menu->description }}</p>
                                            </div>
                                        </div>
                                    </td>
                                    <td class="px-8 py-6 text-center">
                                        <span class="font-mono font-bold text-indigo-600">${{ number_format($menu->price, 2) }}</span>
                                    </td>
                                    <td class="px-8 py-6 text-center">
                                        @if($menu->is_active)
                                    ...
                                    <td class="px-8 py-6 text-right">
                                        <div class="flex justify-end gap-2">
                                            <button class="p-2 text-slate-400 hover:text-indigo-600 hover:bg-slate-100 rounded-lg transition"><svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor"><path d="M13.586 3.586a2 2 0 112.828 2.828l-.793.793-2.828-2.828.793-.793zM11.379 5.793L3 14.172V17h2.828l8.38-8.379-2.83-2.828z" /></svg></button>
                                            <button class="p-2 text-slate-400 hover:text-rose-600 hover:bg-slate-100 rounded-lg transition"><svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor"><path fill-rule="evenodd" d="M9 2a1 1 0 00-.894.553L7.382 4H4a1 1 0 000 2v10a2 2 0 002 2h8a2 2 0 002-2V6a1 1 0 100-2h-3.382l-.724-1.447A1 1 0 0011 2H9zM7 8a1 1 0 012 0v6a1 1 0 11-2 0V8zm5-1a1 1 0 00-1 1v6a1 1 0 102 0V8a1 1 0 00-1-1z" clip-rule="evenodd" /></svg></button>
                                        </div>
                                    </td>
                                </tr>
                            @endforeach
